...
public class PlayerShopSimple extends GenesisAddonConfig {
    ...
    public PlayerShopSimple(PlayerShops plugin, Player owner) {
        super(plugin,
                "shops" + File.separator + owner.getUniqueId().toString().charAt(0) + File.separator
                        + owner.getUniqueId());
        this.plugin = plugin;
    ...
    public void giveReward(Player p) {
        GenesisCurrency c = GenesisCurrency.detectCurrency(plugin.getSettings().getPriceType().name());
        ...
        ClassManager.manager.getMessageHandler()
                .sendMessageDirect(ClassManager.manager.getStringManager()
                        .transform(plugin.getMessages()
                                .get("Message.ReceivedReward")
                                .replace("%reward%", CurrencyTools.getDisplayPrice(c, reward)), p), p);
        reward = 0;
        save();
    }
